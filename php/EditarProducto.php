    ...
    <section class="full-width pageContent">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                // Calcular la ganancia y mostrarla en la tabla
                $ganancia = $row['Cantidad'] * $row['PrecioCompra'] - $row['VenTot'] * $row['PrecioVenta'];
                echo "<td>" . $ganancia . "</td>";
                // Calcular las existencias y mostrarlas en la tabla
                $existencias = $row['Cantidad'] - $row['VenTot'];
                echo "<td>" . $existencias . "</td>";
                echo "<td>";
                // Imagen para borrar producto con formulario
                echo "<form method='post'>";
                echo "<input type='hidden' name='borrar_producto' value='" . $row['Nombre'] . "'>";
                echo "<button type='submit' style='background:none; border:none; padding:0; margin:0;'><img src='../assets/img/borrar.png' alt='Borrar' /></button>";
                echo "</form>";
                echo "</td>";
                // Imagen para editar producto
                echo "<td><a href='Editar.php?nombre=" . urlencode($row['Nombre']) . "'><img src='../assets/img/editar.png' alt='Editar' /></a></td>";
                echo "</tr>";
            }
        } else {
            echo "<tr><td colspan='14'>No hay productos registrados</td></tr>";
        }
        ?>
                    </tbody>
                </table>
